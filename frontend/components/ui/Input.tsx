...
export const Input = forwardRef<HTMLInputElement, InputProps>(
  ({ label, error, className, ...props }, ref) => (
    <div className="flex flex-col gap-1">
      {label && <label className="text-[13px] font-semibold text-anime-text-secondary">{label}</label>}
      <input
        ref={ref}
        className={cn(
          "w-full h-10 bg-anime-bg-surface border border-anime-border rounded-md px-3 text-sm text-anime-text placeholder-anime-text-tertiary",
